...
DropzoneArea.prototype.createFooter = function (first) {
  if (footereditable == 1 && first) {
    ...
    $('.footer-by-default-not-editable').click(function (e) {
      e.preventDefault();
    });
  }
//	else {
...
DropzoneArea.prototype.updateFooter = function (block) {
  for (var i = 0; i < block.length; i++) {
    if (block[i] instanceof TxtBlock) {
      block[i].updateText('<p><span data-redactor="verified" data-redactor-inlinemethods="" style="font-size: 11px;"><span style="font-size: 11px;">Este correo electronico ha sido enviado a %%EMAIL%%</span></span></p><p style="text-align: right;"><span data-redactor="verified" data-redactor-inlinemethods="" style="font-size: 11px;"><a href="%%WEBVERSION%%">Version web</a><span>  | </span></span><span data-redactor="verified" data-redactor-inlinemethods="" style="font-size: 11px;"><a href="%%UNSUBSCRIBE%%">Eliminar suscripcion</a></span></p>');
...
DropzoneArea.prototype.createRow = function (r) {

  var newRow = new rowZone(this,r.listofblocks);
  this.listofrows.push(newRow);
  newRow.createRow();
//  newRow.addBlock(new TxtBlock(newRow));
  
  for (var i = 0; i < r.listofblocks.length; i++) {
    var objPersist = r.listofblocks[i].persist();
    var block;
    if(objPersist.type == 'Text'){
      block = new TxtBlock(newRow);
      block.unpersist(objPersist);
    }
    if(objPersist.type == 'Image'){
      block = new ImgBlock(newRow);
      block.unpersist(objPersist);
    }
    if(objPersist.type == 'Button'){
      block = new BtnBlock(newRow);
      block.unpersist(objPersist);
    }
    if(objPersist.type == 'Social-Share'){
      block = new SShareBlock(newRow);
      block.unpersist(objPersist);
    }
    if(objPersist.type == 'Social-Follow'){
      block = new SFollowBlock(newRow);
      block.unpersist(objPersist);
    }
    newRow.addBlock(block);
  }
//  newRow.updateImagesSize();
