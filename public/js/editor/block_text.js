...
TxtBlock.prototype.setSliderSize = function () {
  var toolbar = $('#my-component-toolbar');
  
  var title = $("<div class='option-title-toolbar'>Tamaño</div>");
  var slider = $("<input type='text' id='sliderToolbar' style='width:100px'>");

  var elements = $('<li class="toolbar-elements"/>');
  elements.append(title);
  elements.append(slider);
  
  toolbar.find('.components-list').append(elements);
  this.activateSlider();
};

TxtBlock.prototype.activateSlider = function () {

  var fullwidth = this.row.dz.widthval / this.row.listofblocks.length;
  var value = Math.floor(this.width / fullwidth * 100);
  var t = this;
  
  $('#sliderToolbar').slider({min: 10, max: 100, value: value, step: 1})
    .on('slide', function (ev) {
      t.content.find('.one-element').css("width",ev.value+'%');
      
//      var width = Math.floor(fullwidth * (ev.value / 100));
//      var height = Math.floor((width * t.height) / t.width);
      t.changeWidth(ev.value);
      console.log(t);
    });
};

TxtBlock.prototype.changeWidth = function (width) {
  console.log(width );
//  console.log(this);
  this.content.find('in-column').css("width",width+'%');
}
