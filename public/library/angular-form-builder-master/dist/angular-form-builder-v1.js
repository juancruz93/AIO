(function () {
  ...
  angular.module('builder.controller', ['builder.provider', 'builder.services'])
          .controller('fbFormObjectEditableController', ['$scope', '$injector', '$uibModal', function ($scope, $injector, $uibModal) {
              var $builder;
...
//                console.log(formObject);
                ...
                $scope.$watch('[label, description, placeholder, required, options, validation,objExt]', function () {
                  ...
                  formObject.objExt = (typeof $scope.objExt == "undefined") ? $scope.objExt = {} : $scope.objExt;
                }, true);
              ...
              return $scope.data = {
                ...
                , backup: function () {
                  /*
                   Backup input value.
                   */
                  var label = angular.copy($scope.label),
                          description = angular.copy($scope.description),
                          placeholder = angular.copy($scope.placeholder),
                          required = angular.copy($scope.required),
                          optionsText = angular.copy($scope.optionsText),
                          validation = angular.copy($scope.validation),
                          objExt = angular.copy($scope.objExt);
                ...
                , rollback: function () {
                  ...
                  $scope.objExt = this.model.objExt;
                }
              };
            }])
          ...
          .controller('fbFormController', ['$scope', '$injector', function ($scope, $injector) {
              ...
              $scope.openDatePicker = function (id) {
                for (var i = 0 in $scope.form) {
                  if ($scope.form[i].id == id) {
                    $scope.form[i].objExt.openen = true;
                  }
                }
              }
          ...
          .controller('fbFormObjectController', ['$scope', '$injector', function ($scope, $injector) {
              ...
              return $scope.updateInput = function (value) {
                ...
                var input;
                input = {
                  component: $scope.formObject.component
                  , id: $scope.formObject.id
                  , label: $scope.formObject.label
                  , value: value != null ? value : ''
                  , objExt: $scope.formObject.objExt
                };
                

                if ($scope.$parent.input.length == $scope.$parent.form.length) {
                  if (typeof $scope.formObject.objExt.anotherAnswer != "undefined" && $scope.formObject.objExt.anotherAnswer == true) {
                    if (value == 'other') {
                      $('#' + $scope.$parent.formName + $scope.formObject.id + 'anotherAnswer').show();
                    } else {
                      $('#' + $scope.$parent.formName + $scope.formObject.id + 'anotherAnswer').hide();
                    }
                  }
                  if (typeof $scope.formObject.objExt.configConditional != "undefined") {
                    for (i in $scope.formObject.objExt.configConditional) {
                      var objExt = {};
                      if ($scope.formObject.objExt.configConditional[i].aswers == value) {
                        objExt.show = true;
                      } else {
                        objExt.show = false;
                      }
                      for (j in $scope.$parent.form) {
                        if ($scope.$parent.form[j].id == $scope.formObject.objExt.configConditional[i].question) {

                          if (typeof $scope.$parent.form[j].isChange == "undefined") {
                            $scope.$parent.form[j].inirequired = $scope.$parent.form[j].required;
                            $scope.$parent.form[j].isChange = true;
                          }

                          if (objExt.show) {
                            if ($scope.$parent.form[j].inirequired != $scope.$parent.form[j].required) {
                              $scope.$parent.form[j].required = $scope.$parent.form[j].inirequired;
                            }
                            $scope.$parent.form[j].objExt.show = true;
                            $('#' + $scope.$parent.formName + $scope.$parent.form[j].id).show();
                          } else {
                            $scope.$parent.form[j].required = false;
                            $scope.$parent.form[j].objExt.show = false;
                            $('#' + $scope.$parent.formName + $scope.$parent.form[j].id).hide();
                            angular.element(document.getElementById($scope.$parent.formName + $scope.$parent.form[j].id)).scope().inputText = "";
                            break;
                          }
                          var auxinput = {
                            component: $scope.$parent.input[j].component
                            , id: $scope.$parent.input[j].id
                            , label: $scope.$parent.input[j].label
                            , value: ""
                            , objExt: objExt
                          };
                          $scope.$parent.input.splice(j, 1, auxinput);
                          break;
                        }
                      }
                    }
          ...
          .controller('ModalInstanceCtrl', function ($uibModalInstance, $scope) {
            ...
            $ctrl.questions = [];
            $ctrl.options = {};
            $ctrl.options.configConditional = [];
            if (typeof $parent.options == "undefined" || $parent.options.length) {
              $uibModalInstance.dismiss('cancel');
            }

            for (i in $parent.$components) {
              if ($parent.$components[i].id != $parent.id && $parent.$components[i].id != "encabezado" && $parent.$components[i].component != "button" && $parent.$components[i].id != "footer" && $parent.$components[i].id != "paragraph") {
                $ctrl.questions.push($parent.$components[i]);
              }
            }
            $ctrl.options.options = $parent.options;
            if(typeof $parent.objExt.configConditional != "undefined"){
              $ctrl.options.configConditional = $parent.objExt.configConditional;
            }
          ...
          .controller('ModalImageCtrl', function ($uibModalInstance, $scope, fbServices) {

            var $parent = $scope.$parent;
            var $parentController = $scope.$parent.$parent.$parent.$parent.$parent;
            var $ctrl = this;
            $ctrl.questions = [];
            $ctrl.options = {};
            $scope.initial = 0;
            ...
            $scope.listForms = function () {
              fbServices.getAssetMedia($scope.initial)
            ...
            $scope.funcUniversal = {
              selectedImage: function ($index) {
                $uibModalInstance.close($scope.imageAccount.items[$index]);
              },
              openUploadImage: function () {
                $uibModalInstance.dismiss('cancel');
                $parentController.toggleRight('right', 4);
              }
            }
...
(function () {
  angular.module('builder.directive', ['builder.provider', 'builder.controller', 'builder.drag', 'validator', 'builder.services'])
          .directive('fbBuilder', ['$injector', function ($injector) {
              var $builder, $drag;
              $builder = $injector.get('$builder');
              ...
              return {
                ...
                , template: $builder.config.templateFbBuilder
...
//                  console.log($(element));
                  return $drag.droppable($(element), {
                    move: function (e, draggable) {
                      ...
                      for (index = _j = 1, _ref1 = positions.length; _j < _ref1; index = _j += 1) {
                        ...
                        if (e.pageY > positions[index - 1] && e.pageY <= positions[index]) {
                          ...
                          if (typeof draggable.object.formObject != "undefined") {
                            if (draggable.object.formObject.id == 'encabezado') {
                              $empty.insertBefore($($formObjects[0]));
                            }
                            if (draggable.object.formObject.component == 'button') {
                              $empty.insertAfter($($formObjects[$formObjects.length - 1]));
                            }
                            if (draggable.object.formObject.component == 'footer') {
                              $empty.insertAfter($($formObjects[$formObjects.length - 1]));
                            }
                          }
                          //                  if (draggable.object.formObject.component == 'encabezado') {
                          //                      $empty.insertBefore($($formObjects[0]));
                          //                    }
                          //                    if (draggable.object.formObject.component == 'button') {
                          //                      $empty.insertAfter($($formObjects[$formObjects.length - 1]));
                    ...
                    , up: function (e, isHover, draggable) {
                      var formObject, newIndex, oldIndex, $formObjects;
                      beginMove = true;
                      if (!$drag.isMouseMoved()) {
                        $(element).find('.empty').remove();
                        return;
                      }
                      //console.log(isHover,draggable);
                      if (!isHover && draggable.mode === 'drag') {
                        formObject = draggable.object.formObject;
                        if (formObject.editable) {
                          if (formObject.component == 'button' || formObject.objExt.noDeleted == true) {
                            return false;
                          } else {
                            $builder.removeFormObject(attrs.fbBuilder, formObject.index, formObject);
                          }
                        }
                      } else if (isHover) {
                        if (draggable.mode === 'mirror') {
                          $builder.insertFormObject(scope.formName, $(element).find('.empty').index('.fb-form-object-editable'), {
                            ...
                            objExt: draggable.object.objExt
                          });
                        }
          ...
          .directive('fbFormObjectEditable', ['$injector', 'fbServices', function ($injector, fbServices) {
              ...
              return {
                ...
                , link: function (scope, element) {
                  var popover;
                  scope.inputArray = [];
                  scope.$components = $builder.forms[scope.$parent.formName];
                  ...
                  scope.$watch('$component.popoverTemplate', function (template) {
                    ...
                    return $(element).popover({
                      html: true
                      , title: scope.formObject[$builder.config.titlePopover]
                  ...
                  scope.popover = {
                    ...
                    , remove: function ($event) {
                       ...
                       */
                      $event.preventDefault();
                      $builder.removeFormObject(scope.$parent.formName, scope.$parent.$index, scope.$parent.object);
                  ...
                  scope.showModalImage = {
                    show: function ($event) {
                      $event.preventDefault();
                      $(element).popover('hide');
                      var Modal = scope.$uibModal.open({
                        controller: 'ModalImageCtrl'
                        , controllerAs: '$ctrl'
                        , scope: scope
                        , ariaLabelledBy: 'modal-title'
                        , ariaDescribedBy: 'modal-body'
                        , template: $builder.config.templateModalImage
                        , size: 'lg'
                      });
                      Modal.result.then(function (newSrcImage) {
//                        console.log(newSrcImage);
                        scope.formObject.objExt.srcImage = newSrcImage.image;
                        scope.setupScope(scope.formObject);
                        scope.popover.save($event);
                      }, function () {

                      });
                    }
                  }
                  scope.conditionalModal = {
                    show: function ($event) {
                      $event.preventDefault();
                      $(element).popover('hide');
                      var Modal = scope.$uibModal.open({
                        ...
                        , template:
                                ...
                                '<h3 class="modal-title" id="modal-title">Logica condicional</h3>' +
                                '</div>' +
                                ...
                                '</thead>' +
                                '<tbody>' +
                                '<tr ng-repeat="option in $ctrl.options.options track by $index" ng-init="$ctrl.options.configConditional[$index].aswers = option">' +
                                '<th>{{option}}</th>' +
                                '<th><select ng-model="$ctrl.options.configConditional[$index].question" class="form-control" ng-options="question.id as question.label for question in  $ctrl.questions "></select></th>' +
                                '</tr>' +
                                '</tbody>' +
                                '</table>' +
                                '</div>' +
                                ...
                                '<button class="btn btn-primary margin-right-5px" ng-click="$ctrl.save()">Guardar</button><button class="btn btn-default margin-right-5px" ng-click="$ctrl.cancel()">Cerrar</button>' +
                                '</div>'
                        , size: 'lg'
                      });
                      Modal.result.then(function (newConfigConditional) {
                        for (i in scope.$components) {
                          for (j in newConfigConditional.configConditional) {
                            if (scope.$components[i].id == newConfigConditional.configConditional[j].question) {
                              scope.$components[i].objExt.isConditional = {};
                              scope.$components[i].objExt.isConditional.value = newConfigConditional.configConditional[j].aswers;
                              scope.$components[i].objExt.isConditional.question = scope.id;
                              break;
                            }
                          }
                        }
                        scope.objExt.configConditional = newConfigConditional.configConditional;
                      }, function () {
          ...
          .directive('fbComponent', ['$injector', function ($injector) {
                ...
                , link: function (scope, element) {
                  ...
                  $drag.draggable($(element), {
                    ...
                    , object: {
                      ...
                      objExt: scope.component.objExt
                    }
                  });
                  return scope.$watch('component', function (template) {
                    var view;
                    if (!template) {
                      return;
                    }
                    if (typeof template.isNotTemplate == "undefined" || template.isNotTemplate == false) {
                      view = $compile(template.template)(scope);
                      return $(element).html(view);
                    }
                    //            view = $compile(template.template)(scope);
                    //              return $(element).html(view);
          ...
          .directive('fbForm', ['$injector', function ($injector) {
              return {
                restrict: 'A'
                , require: 'ngModel'
                , scope: {
          ...
          .directive('fbFormObject', ['$injector', function ($injector) {
              ...
              return {
                ...
                , link: function (scope, element, attrs) {
                  ...
                  if (typeof scope.$component.arrayToText != "undefined") {
                    scope.inputArray = [];
...
(function () {
  angular.module('builder.services', [])
          .service('fbServices', ['$q', '$http', function ($q, $http) {
              this.getAssetMedia = function (page) {
                var url = fullUrlBase + "api/sendmail/getassetmediagallery/" + page;
                return $http.get(url);
              }
            }]);
}).call(this);
            ...
            this.dropMode = (function (_this) {
              return function ($element, options) {
                ...
                result = {
                  id: _this.getNewId()
                  , element: $element[0]
                  , down: function (e, draggable) {
                    return $rootScope.$apply(function () {
                      return typeof options.down === "function" ? options.down(e, draggable) : void 0;
                    });
                  }
                  , move: function (e, draggable) {
                    return $rootScope.$apply(function () {
...
//                  return;
            ...
            this.$get = this.get;
            this.setConfigData = function (item, data) {
              this.data[item] = data;
            }
          });
}).call(this);
  ...
  angular.module('builder.provider', [])
          .provider('$builder', function () {
            ...
            this.config = {
              ...
              , templateFbBuilder: "<div class='form-horizontal'>\n    <div class='fb-form-object-editable ' ng-repeat=\"object in formObjects\"\n        fb-form-object-editable=\"object\"></div>\n</div>"
            };
            ...
            this.forms = {
              "default": []
              , };
            this.setForm = function (name, obj) {
              this.forms[name] = obj;
            }
            this.convertComponent = function (name, component) {
              var result, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9, _ref10;
              result = {
                name: name
                ...
                , objExt: (_ref10 = component.objExt) != null ? _ref10 : {}
              };
...
//              console.log(formObject);
              ...
              if (typeof formObject.objExt == "undefined") {
                formObject.objExt = {};
              }
//              console.log(angular.copy(formObject));
              result = {
                id: formObject.id
                ...
                , objExt: (_ref8 = formObject.objExt) != null ? _ref8 : formObject.objExt
                , };
              return result;
            };
...
//                  console.log(formObjects[index].id);
//                  angular.element(document.getElementById(formObjects[index].id)).scope().setupScope(formObjects[index]);
                }
                if (this.config.countIndex) {
                  var indexObj = 1;
                  var copy;
                  for (j = 0; j < formObjects.length; j++) {
                    if (typeof formObjects[j].objExt.index !== "undefined") {
                      copy = angular.copy(formObjects[j]);
//                      console.log(angular.copy(copy));
                      copy.objExt.index = indexObj;
                      indexObj += 1;
                      formObjects.splice(j, 1, copy);
                    }
                  }
                }
              };
            ...
            this.insertFormObject = (function (_this) {
              return function (name, index, formObject) {
                ...
                if (typeof formObject.id == "undefined") {
                  formObject.id = id;
                }

                //        formObject = formObjects.splice(oldIndex, 1)[0];
                //        formObjects.splice(newIndex, 0, formObject);
                if (formObject.id == "encabezado") {
                  _this.forms[name].splice(0, 0, _this.convertFormObject(name, formObject));
                } else {
                  if (formObject.component == "button") {
                    let length = _this.forms[name].length - 1;
                    if (_this.forms[name].length > 0 && typeof _this.forms[name][length].id != "undefined" && _this.forms[name][length].id == "footer") {
                      _this.forms[name].splice(_this.forms[name].length, 0, _this.convertFormObject(name, formObject));
                    } else {
                      _this.forms[name].splice(_this.forms[name].length + 1, 0, _this.convertFormObject(name, formObject));
                    }
                  } else {
                    if (formObject.id == "footer") {
                      _this.forms[name].splice(_this.forms[name].length + 1, 0, _this.convertFormObject(name, formObject));
                    } else {
                      let lengthForm = _this.forms[name].length;
                      if (index == lengthForm) {

                        if(typeof _this.forms[name][lengthForm-2] != "undefined" && _this.forms[name][lengthForm-2].component == "button"){
                          index = lengthForm - 2;
                        }else{
                          index = lengthForm - 1;
                        }
                      }else if(index == (lengthForm-1)){
                        if(_this.forms[name][lengthForm-2].component == "button"){
                          index = lengthForm - 2;
                        }
                      }else if(index == 0){
                        index = 1;
            ...
            this.removeFormObject = (function (_this) {
              return function (name, index, objIndex) {
                /*
                ...
                if (typeof objIndex != "undefined") {
                  if (typeof objIndex.objExt != "undefined") {
                    if (typeof objIndex.objExt.dependence != "undefined") {
                      for (var i in formObjects) {
                        if (objIndex.objExt.dependence == formObjects[i].id) {
                          formObjects.splice(formObjects[i].index, 1);
                          if (formObjects[i].index > index) {
                            formObjects.splice(index, 1);
                          } else {
                            formObjects.splice(index - 1, 1);
                          }
                        }
                      }
                    } else {
            ...
            this.updateFormObjectIndex = (function (_this) {
              return function (name, oldIndex, newIndex) {
                ...
                formObjects = _this.forms[name];
                if (formObjects[newIndex].id == 'encabezado' || formObjects[newIndex].component == 'button' || formObjects[newIndex].id == 'footer') {
                  return;
                }
                formObject = formObjects.splice(oldIndex, 1)[0];
                formObjects.splice(newIndex, 0, formObject);
            ...
            this.updateConfig = (function (_this) {
              return function (name, nameobj, valueobj) {
                /*
                 Update the index of the form object.
                 @param name: The form name.
                 @param oldIndex: The old index.
                ...
                for (var i in formObjects) {
                  ...
                  if (formObject.id != 'encabezado' && formObject.component != 'button') {
                    formObject.objExt[nameobj] = valueobj;
                    //angular.element(document.getElementById(formObject.id)).scope().setupScope(formObject);
                  }
                }
//                console.log(angular.element(document.getElementById(formObjects.id)).scope());
              ...
              '$injector', (function (_this) {
                return function ($injector) {
                  ...
                  return {
                    ...
                    , updateConfig: _this.updateConfig
