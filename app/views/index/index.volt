...
{% block css %}
{{style_sheet_link('css/button_help.css')}}
  {# Notifications #}
  ...
  <style>
    ...
    .row-prueba > [class*='col-'] {
      display: flex;
      flex-direction: column;
    }
    .img-static-dashboard{
      padding-bottom:5px;
      height: 150px;
      width: 70%;
      max-width: 100%;
...
{% block js %}
  ...
  {{ javascript_include('js/angular/dashboardconfig/directive.js') }}
  {{ javascript_include('js/angular/dashboardconfig/service.js') }}
  {{ javascript_include('js/angular/dashboardconfig/controller.js') }}
  {{ javascript_include('js/angular/dashboardconfig/app.js') }} 
{% endblock %}
{% block content %}
  <div class="clearfix"></div>
  <div class="dashboard-abjust"></div>     
  {% if(user.UserType.idSubaccount) %}
    <div class="row" ng-controller="dashboard" ng-init="getConfigAccount({{user.UserType.Subaccount.idAccount}})" ng-cloak>
<div class="modal fade" id="adjun" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <label class="modal-title" id="myModalLabel" style="font-size: 18px">Configurar registros DKIM</label>
      </div>
        ...
        <div role="tabpanel">
          ...
          <div class="tab-content">
            <div class="row" style="padding-left: 10px;">
              <div class="col-lg-12">
                <label for="name" class="col-xs-12" style="padding-top: 10px; padding-left: 0px;font-size: 18px;font-weight:normal">Si quieres mejorar tu ín​dice de lecturas en tus campañas de email marketing, te aconsejamos autentificar y firmar tus emails, consiguiendo una mayor tasa de apertura mediante la firma DKIM, solucionando la duda de los filtros antispam demostrando la autenticidad del emisor.</label>      
              </div>
              <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9">
                <label for="inputEmail4">Ingresa el dominio a configurar: </label>
                <input type="text" class="form-control mb-2 mr-sm-2" id="domain" ng-model="domain" placeholder="midominio.com">
                <h6 class="color-danger" ng-show="validatedomain">
                   Debe ingresar el dominio a configurar
                  </h6>
                  <h5>
                   Te enviaremos un correo a tu email con el paso a paso de la configuración
                  </h5>
              </div>
            </div>
            <div class="row" style="padding-left: 10px;">
              <div class="col-lg-12 text-right">
                <button type="button" class="button shining btn btn-md danger-inverted"  ng-click="closeModalDKIM()">Cerrar</button>
                <button type="button" class="button shining btn btn-md success-inverted" ng-click="save()">Crear</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 none-paddign" ng-if="arrConfigDashboard.items.length > 0">
        <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9 none-paddign">
          <div  class="col-xs-12 col-sm-12 col-md-3 col-lg-3 heigth-50-porc "  ng-repeat="item in arrConfigDashboard.items track by $index">
            <div class="dashboard-image-center center-block cursor-pointer" ng-click="go(item.hrefEnlace)">
              <img class="img-static-dashboard" ng-src="{{'{{item.imageDashboard}}'}}" alt="Crear lista de contactos" >
            </div>
            <div class="dashboard-item-title-center ">
              <a ng-href="{{'{{item.hrefEnlace}}'}}" class="small-text cursor-pointer">{{"{{item.textEnlace}}"}}</a>
            </div>
          </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 none-paddign ">
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 none-paddign">
            <div class="dashboard-image-center center-block img-100-width" >
              <img class="img-100-width" ng-src="{{'{{arrConfigDashboard.topImage}}'}}" >
            </div>
          </div> 
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 item-dasboard none-margin-left none-padding-left">
            <div class="dashboard-item-image-center img-100-width" >
              <img class="img-100-width" ng-src="{{'{{arrConfigDashboard.bottomImage}}'}}" >
            </div>
          </div>
        </div>
      </div>
  ...
  {% elseif(user.UserType.idAccount) %}
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
        <div class="row">
          <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4 dashboard-item">
            <div onclick="location.href = '{{url('reports/index')}}'" class="dashboard-menu-item">
              <div class="dashboard-item-image">
                <img src="{{url('')}}images/general/estadisticas.png" alt="Estadísticas y reportes" />
              </div>
              <div class="dashboard-item-text">
                <p>
                  Aquí podrás consultar y descargar de manera global los reportes de las actividades que haz realizado en la plataforma, como también la actividad de tus contactos respecto al contenido que les hayas enviado.
                </p>
              </div>

              <div class="dashboard-item-title">
                <a href="{{url('reports/index')}}"  class="small-text">Estadísticas y reportes</a>
              </div>
            </div>
          </div>

          <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4 dashboard-item">
            <div onclick="location.href = '{{url('gallery')}}';" class="dashboard-menu-item">
              <div class="dashboard-item-image">
                <img src="{{url('')}}images/general/galeria.png" alt="Galería de imágenes" />
              </div>
              <div class="dashboard-item-text">
                <p>
                  Puedes gestionar tu galería de imágenes y archivos de manera rápida y fácil, puedes cargar y borrar archivos que luego podrás usar en el resto de servicios de la plataforma.
                </p>
              </div>

              <div class="dashboard-item-title">
                <a href="{{url('gallery')}}" class="small-text">Ver galería de imágenes</a>
              </div>
            </div>
          </div>
        </div>
  ...
  <div id="container-floating">
    ...
    <div class="nd1 nds" data-toggle="tooltip" data-placement="left" title="Youtube"><img class="reminder">
      ...
      <!-- <a href="https://www.youtube.com/playlist?list=PLruewKCYa5VvmHk_nJP_CcA1ahIWmTurD" target="_blank"> -->
