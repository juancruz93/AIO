...
{% block content %}
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
            <div class="title">
                Editar el lote <strong>{{ sms.name }}</strong>
            </div>
            <hr class="basic-line"/>
        </div>
    </div>

    <div class="row" ng-app="aio" ng-controller="ctrlSms" ng-init='getOne({{ sms.idSms }})'>
                                    ...
                                    <span class="input-default col-sm-8 col-md-8">
                                        <select id="idSmsCategory" name="idSmsCategory" ng-model="idSmsCategory" class="select2" required="">
                                            {% for item in category %}
                                                {% if item.idSmsCategory == sms.idSmsCategory%}
                                                    <option value="{{ item.idSmsCategory }}" selected>{{ item.name }}</option>
                                                {% else %}
                                ...
                                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
                                    ...
                                    <span class="input-default col-sm-8 col-md-8">
                                        {#       <select id="timezone" name="timezone" class="select2" required="true" ng-model="timezone">
                                                 <option ng-repeat="zone in timezones" value="{{ "{{zone.gmt}}" }}" ng-selected="zone.gmt == gmt">{{ "{{zone.gmt == gmt}}" }} // {{"{{gmt}}"}} //{{ "{{zone.countries}}" }}</option>
                                               </select>#}

                                        <select id="timezone" name="timezone" ng-model="gmt" class="select2" >
                                            {% for item in timezones %}
                                                {% if item['gmt'] == sms.gmt%}
                                                    <option value="{{ item['gmt'] }}" selected>{{ item['countries'] }}</option>
                                                {% else %}
                                                    <option value="{{ item['gmt'] }}" >{{ item['countries'] }}</option>
                                                {% endif %}
                                            {% endfor %}
                                        </select>
                                    </span>
                            ...
                            <div class="form-group" ng-show="!datenow">
                                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
                                    ...
                                    <span class="input-append date  col-sm-8 col-md-8 input-group" id='datetimepicker' style="padding-left: 15px; padding-right: 15px;">
                                        {{ smsloteform.render('startdate', { 'readonly':'', 'class': 'undeline-input' , 'id': 'datesend', 'required' : 'required' , 'keep-current-value':'' , 'ng-model': 'originalDate' }) }}
                                ...
                                <div class="form-group" >
                                    ...
                                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap"> 
                                        ...
                                        <span class="input hoshi input-default col-sm-8 col-md-8">
                                            ...
                                            <h6 class="color-warning" ng-show='existTags'>Parece ser que está usando etiquetas. Recuerde que en esta modalidad <b>no se puede usar etiquetas</b>, así que el mensaje llegará tal cual con los caracteres usados</h6>
            ...
            <div class="fill-block fill-block-info" >
                ...
                <div class="body">
                    ...
                    <ul>
                        ...
                        <li>La fecha y hora de envío es para decidir cuando se va a enviar los SMS y tiene que ser entre las 7:00  y las 18:00 .</li>
                        <li>El envío de notificaciones por defecto esta en "No" para activarlo haga clic en el switch para que cambie a "Si"</li>
                        <li>Los correos se deben de separar por coma "," maximo 8</li>
                        <li>Los destinatarios es a quien va a enviarse los SMS y tienen que ir de la siguiente forma: codigo de pais(sin el simbolo "+"), celular, mensaje. Separados por coma ","</li>
            ...
            <div class="dialog__content">
                ...
                <div class="dialog-inner">
                    ...
                    <div>                    
                        ...
                        <a ng-click="editlote()"  id="btn-ok" class="button shining btn btn-md success-inverted">Confirmar</a>
                    </div>
                </div>
            </div>
        </div>
