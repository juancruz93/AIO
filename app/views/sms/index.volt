...
{% block js %}
  ...
  <script type="text/javascript">
    ...
    var idSubaccountLogin
    = {{user.Usertype.idSubaccount}};
      var loadGetAll = '';
  </script>
  <script type="text/javascript">
    $(function () {
    ...
    {#      console.log(idSms);#}
    {#      $("#btn-ok-cancel").attr('href', myURL);#}
        $("#btn-ok-cancel").attr('data-idsms', idSms);
      });

    {#$(document).on("click","btn-ok-cancel",function(){
        alert("hola");
        console.log($(this).data('idsms'));
        cancelSms($(this).data('idsms'));
    });#}
      ...
      function openModaEditl() {
        $('#somedialogEdit').addClass('dialog--open');
      }
...
{% block content %}
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
      ...
      <hr class="basic-line" />
      <p>
        En esta lista podrá ver, editar y eliminar los envíos de  SMS
      </p>
    </div>
  </div>

  <div id="messages"></div>
  <div ng-app="aio" ng-controller="ctrlSms" ng-cloak>
    <div class="row" style="display: flex; justify-content: center;" ng-init="getCategory()">
      <div class="col-xs-4 col-md-2" style="margin-left: 30px">
        <div class="input-group">
          <input class="form-control"  id="name" ng-change='search()' placeholder="Buscar nombre" ng-model="filter.name" />
      ...
      <div class="col-xs-4 col-md-2">
        <div class="input-group">
          <span class="input-group-addon cursor" id="basic-addon1" placement="top" data-toggle="popover" title="" data-content="Puede escribir un número celular para buscar y luego oprimir tecla 'Enter' de su teclado."  data-original-title="Instrucciones">
            <i class="fa fa-question-circle" aria-hidden="true"></i>
          </span>
          <input type = "number" min="1" max="9999999999" oninput="maxLengthCheck(this)" class="form-control" ng-keydown="$event.keyCode === 13 && search()" id="phoneNumber" placeholder="Buscar celular" ng-model="filter.phoneNumber"/>
          {#          <span class=" input-group-addon" id="basic-addon1" >
                      <i class="fa fa-search"></i>
                    </span>#}
        </div>
      ...
      <div class="col-xs-4 col-md-3">
        <ui-select ng-change='searchcategory()' multiple ng-model="filter.category" ng-required="true"  ui-select-required 
          ...
          <ui-select-choices repeat="key.idSmsCategory as key in smsCategory | propsFilter: {name: $select.search}">
            <div ng-bind-html="key.name | highlight: $select.search"></div>
          </ui-select-choices>
        </ui-select>
      </div>
      <div class="col-xs-4 col-md-2" {#style="margin-left: 65px"#} >
        <select class="form-control " name="smsStatus" ng-model="filter.smsStatus" ng-change='statusFunc()'>
        ...
        <div class="col-xs-6 col-md-6 text-right wrap form-inline">
        <div class="input-group"
    ...
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap text-right">
        ...
        <a href="{{url('smstemplate#/')}}" class="button btn button primary-inverted">Plantillas de SMS</a>
        <a href="{{url('smscategory#/')}}" class="button btn button warning-inverted">Categorías de SMS</a>
        <a href="{{url('sms/tools')}}" class="button btn button success-inverted">Crear envío de SMS</a>
      </div>
    </div>
    <br>
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      <div ng-class="{'hidden' : misc.progressbar}" style="margin-left: 10px">
    ...
    <div id="pagination" class="text-center">
      <ul class="pagination">
        ...
        <li   ng-class="page == (sms.total_pages) || sms.total_pages == 0 ? 'disabled'  : ''">
          <a href="#/" ng-click="page == (sms.total_pages)  || sms.total_pages == 0  ? true  : false || page == (sms.total_pages)  || sms.total_pages == 0  ? true  : false || forward()" class="new-element"><i class="glyphicon glyphicon-step-forward"></i></a>
        </li>
        <li   ng-class="page == (sms.total_pages)  || sms.total_pages == 0 ? 'disabled'  : ''">
          <a ng-click="page == (sms.total_pages)  || sms.total_pages == 0  ? true  : false || fastforward()" class="new-element"><i class="glyphicon glyphicon-fast-forward"></i></a>
        </li>
      </ul>
    </div>
    <div class="row" >
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
        <table class="table table-bordered sticky-enabled" >
          ...
          <tbody>
            <tr ng-repeat="item in sms.items">
              <td class="border-left-{{'{{item.status}}'}}-5px">
                <strong class=" ng-binding medium-text ">
                  <span ng-class="item.type == 'automatic' ? 'fa fa-cogs' : (item.birthdate ? 'fa fa-birthday-cake' : '')""></span>
                  {{'{{item.name}}'}}
                </strong>
                <dl>
                  <dd class="small-text" type-sms="{{'{{item.type}}'}}"><b></b></dd>
                  <dd class="small-text" status-sms="{{'{{item.status}}'}}"><b></b>  </dd>
                  <dd> <em class="extra-small-text">Creado por <strong>{{'{{item.createdBy}}'}}</strong> , a las <strong>{{'{{item.created}}'}}</strong> </em></dd>
                  <dd> <em class="extra-small-text">Actualizado por <strong> {{'{{item.updatedBy}}'}}</strong>, a las  <strong>{{'{{item.updated}}'}}</strong></em></dd>
                </dl>
              </td>
              <td>
                {{'{{item.categoria}}'}}
              ...
              <td>
                ...
                <div>
                  ...
                  <div ng-if="item.sendpush ==1">
                    <dd>Mensaje flash: <b>Si</b></dd>
                  </div>
                  <div ng-if="item.sendpush ==0">
                    <dd>Mensaje flash: <b>No</b></dd>
                  </div>
                  <dd>Fecha de envio: {{'{{item.startdate}}'}}</dd>
                  <dd>Cantidad de SMS: {{'{{item.target}}'}}</dd>
                  {#                  <dd>Cantidad de SMS: {{'{{item.total_sent}}'}}</dd>#}
                  <dd ng-if="item.sent > 0">Cantidad de SMS Enviados: {{'{{item.sent}}'}}</dd>
                  <dd ng-if="item.sent > 0">Cantidad para cobro: {{'{{item.messageCount}}'}}</dd>
                  {#                  <dd ng-if="item.total > 0">Cantidad de SMS Total: {{'{{item.total}}'}}</dd>#}
                </div>
                
              </td>
              <td>
                <a  ng-if="item.status == 'scheduled'"   id="cancel" ng-attr-data-idSms="{{'{{item.idSms}}'}}" ng-attr-data-id="{{url('sms/smscancel')}}/{{'{{item.idSms}}'}}" onclick="openModalCancel($(this).data('idsms'))" class="button btn btn-xs-round danger-inverted" data-toggle="tooltip" data-placement="top" title="Cancelar envio">
                  <span class="fa fa-ban"></span>
                </a>
                <div  ng-if="item.status == 'sent' && item.type == 'contact'">
                  {#<a  href="{{url('sms/showcontact')}}/{{'{{item.idSms}}'}}" class="button btn btn-xs-round default-inverted" data-toggle="tooltip" data-placement="top" title="Información detallada del envío">
                    <span class="glyphicon glyphicon-eye-open"></span> 
                  </a>#}
                  <a href="{{url("statistic#/sms")}}/{{'{{item.idSms}}'}}" class="button btn btn-xs-round primary-inverted" data-toggle="tooltip" data-placement="top" title="Ver estadísticas">
                    <span class="fa fa-bar-chart"></span>
                  </a>
                </div>
                <div  ng-if="item.status == 'sent' && item.type != 'contact'">
                  <a href="{{url('sms/showlote')}}/{{'{{item.idSms}}'}}" class="button btn btn-xs-round default-inverted" data-toggle="tooltip" data-placement="top" title="Información detallada del envío">
                    <span class="glyphicon glyphicon-eye-open"></span>
                  </a>
                  <a href="{{url("statistic#/sms")}}/{{'{{item.idSms}}'}}" class="button btn btn-xs-round primary-inverted" data-toggle="tooltip" data-placement="top" title="Ver estadísticas">
                    <span class="fa fa-bar-chart"></span>
                ...
                <div  ng-if="item.status == 'sending' ">
                  <button onclick="return pauseSms(this.value)" ng-value="{{'{{item.idSms}}'}}" class="button btn btn-xs-round warning-inverted" data-toggle="tooltip" data-placement="top" title="Pausar envio">
                    <span class="glyphicon glyphicon-pause"></span>
                  </button>
                  <button onclick="return openModalCancel(this.value)" ng-value="{{'{{item.idSms}}'}}" class="button btn btn-xs-round danger-inverted" data-toggle="tooltip" data-placement="top" title="Cancelar envio">
                    <span class="fa fa-ban"></span>
                  </button>
                </div>
                <div  ng-if="item.status == 'paused' ">
                  <button ng-if="item.status == 'paused'" onclick="return resumeSms(this.value)" ng-value="{{'{{item.idSms}}'}}" class="button btn btn-xs-round success-inverted" data-toggle="tooltip" data-placement="top" title="Reanudar envio">
                    <span class="glyphicon glyphicon-play"></span>
                  </button>
                </div>
                {#<button ng-if="item.status == 'draft'" id="delete" onClick="openModal();" class="button shining btn btn-xs-round shining shining-round round-button danger-inverted" data-toggle="tooltip" data-placement="top" title="Cancelar envio" data-id="{{url('sms/smscancel')}}/{{'{{item.idSms}}'}}">
                  <span class="fa fa-ban"></span>
                </button>#}
                <a ng-if="item.type == 'contact' && (item.status == 'draft' || item.status == 'scheduled')" href="{{url('sms/editcontact')}}/{{'{{item.idSms}}'}}" class="button btn btn-xs-round info-inverted" data-toggle="tooltip" data-placement="top" title="Editar envio de sms">
                  <span class="glyphicon glyphicon-pencil"></span>
                </a>
                <a ng-if="item.type != 'contact' && (item.status == 'draft' || item.status == 'scheduled')"href="{{url('sms/edit')}}/{{'{{item.idSms}}'}}" class="button btn btn-xs-round info-inverted" data-toggle="tooltip" data-placement="top" title="Editar envio de sms">
                  <span class="glyphicon glyphicon-pencil"></span>
                </a>
              </td>
            </tr>
      ...
      <div ng-if="sms.items.length<=0">
        <div ng-if="misc.progressbar" class="row">
          <div ng-disabled class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
            <div class="block block-success" style="margin: 15px">
              <div class="body success-no-hover text-center">
                <h2>
                  No existen envíos de SMS actualmente, si desea crear uno por lote haga <a href="{{url('sms/createlote')}}">click aquí</a>,
    ...
    <div id="cancelDialog" class="dialog">
      ...
      <div class="dialog__content">
        ...
        <div class="dialog-inner">
          ...
          <div style="z-index: 999999;">
            <a onClick="closeModal();" class="button shining btn btn-md danger-inverted" data-dialog-close>Cancelar</a>
            <button data-idsms="" onclick="return cancelSms()" id="btn-ok-cancel" class="button shining btn btn-md success-inverted">Confirmar</button>
          </div>
        </div>
      </div>
    </div>
