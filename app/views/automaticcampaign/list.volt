...
<div class="row">
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
    <div class="title">
      Listado de campañas automáticas
    </div>            
    <hr class="basic-line" />
    <p>
      ¿Necesitas que tus servicios colaboren entre sí y se activen automáticamente? Con ésta funcionalidad podrás enviar una campaña de correo y hacer que después de una acción como una apertura se envíe un mensaje SMS automáticamente.
    </p>
  </div>
</div>
...
<div class="row">
  ...
  <div class="col-xs-3 col-sm-3 col-lg-3">
    <ui-select ng-change='searchcategory()' multiple ng-model="filter.category" ng-required="true"  ui-select-required 
      ...
      <ui-select-choices repeat="key.id as key in automaCategory | propsFilter: {name: $select.search}">
...
<div class="row">
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
    <div class="pull-right ">
      ...
      <a class="btn button warning-inverted" href="automaticcampaigncategory#/">Categorías de campaña automática</a>
      <a class="btn button success-inverted" ui-sref="create">Nueva campaña automática</a>
    </div>
  </div>
</div>

<div class="row" ng-show="listAutomaticCampaign.items.length > 0">
  <div id="pagination" class="text-center" >
    <ul class="pagination">
      ...
      <li>
        <span><b><script id="metamorph-58-start" type="text/x-placeholder"></script>{{ "{{listAutomaticCampaign.total }}"}}
          </b> registros </span><span>Página <b>{{"{{ page }}"}}
          </b> de <b>
            {{ "{{ (listAutomaticCampaign.total_pages ) }}"}}
          </b></span>
      </li>
      <li   ng-class="page == (listAutomaticCampaign.total_pages)  || listAutomaticCampaign.total_pages == 0  ? 'disabled'  : ''">
        <a href="#/" ng-click="page == (listAutomaticCampaign.total_pages)  || listAutomaticCampaign.total_pages == 0  ? true  : false || forward()" class="new-element"><i class="glyphicon glyphicon-step-forward"></i></a>
      </li>
      <li   ng-class="page == (listAutomaticCampaign.total_pages)  || listAutomaticCampaign.total_pages == 0  ? 'disabled'  : ''">
        <a ng-click="page == (listAutomaticCampaign.total_pages)  || listAutomaticCampaign.total_pages == 0  ? true  : false || fastforward()" class="new-element"><i class="glyphicon glyphicon-fast-forward"></i></a>
      </li>
    </ul>
  </div>
  {#<div class="row">#}
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap" >
    <div class="fill-block fill-block-default">
      ...
      <div class="body">
        <div ng-repeat="key in listAutomaticCampaign.items" class="row" >
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 " style="float: left; width: 100%">
            <div ng-class="{'mailTest':key.test == 1,'border-left-success-10px': key.status == 'completed','border-left-default-10px': key.status == 'draft','border-left-violet-10px': key.status == 'executing','border-left-info-10px': key.status == 'confirmed','border-left-warning-10px': key.status == 'disabled','border-left-danger-10px': key.status == 'canceled'}" class="row fill-block fill-block-default" >
              <div class="row ">
                <div class="col-xs-4 col-sm-4 col-lg-4 cursor" data-toggle="collapse" data-target="#allinfo{{ '{{key.idAutomaticCampaign}}' }}" aria-expanded="false" 
                     aria-controls="allinfo{{ "{{ key.idAutomaticCampaign }}"}}">
                  <di>
                    <dd> <u>{{"{{key.name}}"}}</u></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'completed'">Completado</em></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'canceled' && key.canceleduser == null">Cancelado</em></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'canceled' && key.canceleduser != null">Cancelado por: <b>{{"{{key.canceleduser}}"}}</b></em></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'draft'">Borrador</em></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'confirmed'">Programado</em></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'paused'">Pausado</em></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'executing'">En proceso </em></dd>
                    <dd><em class="extra-small-text" ng-show="key.status == 'birthday'">Cumpleaños</em></dd>
                  </di>
                </div>
                <div class="col-xs-4 col-sm-4 col-lg-4 row" data-toggle="collapse" data-target="#allinfo{{ '{{key.idAutomaticCampaign}}' }}" aria-expanded="false" 
                     aria-controls="allinfo{{ "{{ key.idAutomaticCampaign }}"}}">
                  <div class="col-lg-6 col-md-6 col-sm-6">
                    <di>
                      <dd> <b>
                          Destinatarios: 
                          {{"{{key.quantitytarget.count}}"}}
                        </b></dd>
                      <dd>
                      <dd><em class="extra-small-text">Programado {{"{{ ((key.startDate) ? key.startDate : 'No programado') }}"}}</em></dd>
                      </dd>
                    </di>
                  </div>
                  <div class="col-lg-6 col-md-6 col-sm-6">
                ...
                <div class="col-xs-4 col-sm-4 col-lg-4 text-right">
                  ...
                  {#<a href="{{url('mail/clonemail')}}/{{"{{key.idMail}}"}}" class="button btn btn-xs-round primary-inverted" >
                    <md-tooltip md-direction="bottom">
                      Duplicar correo
                    </md-tooltip>
                  ...
                  {#<button type="button" class="button btn btn-xs-round violet-inverted" data-ng-click="previewmailtempcont(key.idMail);" data-toggle="modal" data-target="#myModal">
                    <md-tooltip md-direction="bottom">
                      Previsualizar
                    </md-tooltip>
                  ...
                  {#<a ng-click="cancelMailAn(key.idMail)" ng-show="key.status == 'sending' || key.status == 'paused' || key.status == 'scheduled'" class="button btn btn-xs-round danger-inverted" >
                    <md-tooltip md-direction="bottom">
                      Cancelar envio
                    </md-tooltip>
                  ...
                  <a ui-sref="edit({idautomaticcampaign:key.idAutomaticCampaign})" ng-show="key.status == 'draft' || key.status == 'confirmed'" class="button shining btn btn-xs-round shining shining-round round-button info-inverted" >
                    <md-tooltip md-direction="bottom">
                      Editar campaña automatica
                    </md-tooltip>
                    <span class="glyphicon glyphicon-pencil"></span>
                  </a>
                  <a href="{{url('apiautomaticcampaign/cloneca')}}/{{"{{key.idAutomaticCampaign}}"}}" class="button shining btn btn-xs-round shining shining-round round-button primary-inverted">
                    <md-tooltip md-direction="bottom">
                      Duplicar Campaña
                    </md-tooltip>
                    <i class="fa fa-files-o"></i>
                  </a>
                  <a ui-sref="viewscheme({idautomaticcampaign:key.idAutomaticCampaign})" class="button shining btn btn-xs-round shining shining-round round-button default-inverted">
                    <md-tooltip md-direction="bottom">
                      Ver esquema de la campaña
                    </md-tooltip>
                    <i class="fa fa-eye"></i>
                  </a>
                  <a href="{{url("statistic#/automaticcampaign")}}/{{"{{key.idAutomaticCampaign}}"}}"  {#ng-show="key.status == 'sent'"#}  class="button shining btn btn-xs-round shining shining-round round-button success-inverted" >
                    <md-tooltip md-direction="bottom">
                      Ver estadísticas
                    </md-tooltip>
                  ...
                  <a href="" ng-show="key.status == 'executing' || key.status == 'confirmed'" class="button shining btn btn-xs-round shining-round round-button danger-inverted" data-ng-click="openModal(key.idAutomaticCampaign)">
                    <md-tooltip md-direction="bottom">
                      Cancelar campaña automática
                    </md-tooltip>
                    <span class="fa fa-ban"></span>
                  </a>

                  {#<a href="{{url("statistic#/mail")}}/{{"{{key.idMail}}"}}"  ng-show="key.status == 'sent'"  class="button shining btn btn-xs-round shining shining-round round-button success-inverted" >
                    <md-tooltip md-direction="bottom">
                      Ver estadísticas
                    </md-tooltip>
                    <span class="fa fa-bar-chart"></span>
...
<div ng-show="listAutomaticCampaign.items.length == 0">
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
    <div class="block block-success">
      <div class="body success-no-hover text-center">
        <h2>
          No hay registros de campañas automáticas, para crear una haga <a ui-sref="create"><u>Click aquí</u></a>.
        </h2>
        </h2>
      </div>
    </div>
  ...
  <div class="dialog__content">
    ...
    <div class="dialog-inner dialog-padding">
      <div class="body row text-center">
        <h3>¿Está seguro que desea cancelar la campaña automática?</h3>
      </div>
      <div class="body row" style="padding-top: 1em;">                    
        <a ng-click="closeModal()" class="button shining btn btn-md danger-inverted" data-dialog-close>Cerrar</a>
        <a ng-click="cancelCampaign()"  id="btn-ok" class="button shining btn btn-md success-inverted">Confirmar</a>
      </div>
    </div>
  </div>
</div> 
<div id="container-floating">
  <div class="nd1 nds" data-toggle="tooltip" data-placement="left" title="Youtube"><img class="reminder">
    ...
    <a href="https://www.youtube.com/playlist?list=PLruewKCYa5VvAsgjiSsEmNIAGsqDuDiKT" target="_blank">
