...
{% block js %}
  ...
  {#{{ javascript_include('js/angular/survey/app.js?v=1.0.2') }}
  {{ javascript_include('js/angular/survey/controllers.js?v=1.0.2') }}
  {{ javascript_include('js/angular/survey/services.js?v=1.0.2') }} #}
   {{ javascript_include('js/angular/survey/dist/survey.a41a346873d5b5e1c1b6.min.js') }} 
  ...
  {{ javascript_include('library/angular-spectrum-colorpicker/dist/angular-spectrum-colorpicker.js') }}
  {{ javascript_include('library/angular-1.5/js/angular-bind-html-compile.min.js') }}
  {{ javascript_include('library/spectrum/js/spectrum.js') }}
  {{ javascript_include('library/bootstrap-datetimepicker-0.0.11/build/js/bootstrap-datetimepicker.min.js') }}
...
{% block content %}
  ...
  <div id="indexController" ng-cloak ng-controller="indexController">
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
        ...
        <hr class="basic-line">
        <p>
           Aquí encontrará el listado de encuestas que se han creado en la plataforma. Podrá encontrar información acerca de la configuración de cada encuesta (Previsualizar, duplicar, editar, generar link). Además podrá ver las estadísticas asociadas a cada una.
    ...
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap ">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 text-right none-padding">
          ...
          <a href="{{ url('surveycategory#/') }}" class="button btn button warning-inverted">Categorías de encuestas</a>
          <a href="{{ url('survey/create#/basicinformation/') }}" class="button shining btn btn success-inverted">Crear una nueva encuesta</a>
        </div>
      </div>
    </div>
    ...
    <div class="row" >
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap" ng-show="list.items.length > 0">
        <table class="table table-bordered sticky-enabled">
          ...
          <tbody>
            <tr data-ng-repeat="i in list.items" data-ng-class="i.status == 'published' ? 'success letter-no-hover': i.status == 'expireded' ? 'danger letter-no-hover':''">
              <td>
              ...
              <td>
                ...
                <p>
                <p>
                  <b>Categoría:</b> {{ "{{ i.surveyCategory }}" }}
                </p>
                <br>
                <p>
                  <b>Cantidad de Respuestas:</b> {{ "{{ i.count }}" }}
                </p>
              ...
              <td>
                <a href="" ng-show="i.count == 0" ng-click="confirmDeleteSurvey(i.idSurvey)" class="button shining btn btn-xs-round shining-round round-button danger-inverted" title="Eliminar encuesta" >
                  <md-tooltip md-direction="bottom">
                    Eliminar encuesta
                  </md-tooltip>
                  <span class="glyphicon glyphicon-trash"</span>
                </a>
                <a href="{{ url('survey/create#/basicinformation/') }}{{ '{{ i.idSurvey }}' }}" class="button shining btn btn-xs-round shining-round round-button info-inverted" title="Editar encuesta">
                  <md-tooltip md-direction="bottom">
                    Editar encuesta
                  </md-tooltip>
                  <span class="glyphicon glyphicon-pencil"></span>
                </a>
                <button type="button" ng-if="i.content != false" class="button btn btn-xs-round default-inverted" data-ng-click="linkGenerator(i.idSurvey)">
                  <md-tooltip md-direction="bottom">
                    Generar link
                  </md-tooltip>
                ...
                <a ng-click="duplicateSurvey(i.idSurvey)" ng-show="i.status == 'published'" class="button btn btn-xs-round primary-inverted" aria-label="Duplicar encuesta">
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
          <div class="block block-success">
            <div class="body success-no-hover text-center">
              <h2>
                No hay registros de categorías que coincidan con los filtros, para crear una haga <a href="{{ url('survey/create#/basicinformation/') }}"><u>Clic aquí</u></a>.
              </h2>
              </h2>
            </div>
          </div>
          ...
          <div class="modal-body">
            ...
            <div class="row">
              <div class="col-sm-12 text-center">
                <button type="button" class="btn danger-inverted" data-dismiss="modal" onclick="javascript:window.location.reload();">Cerrar</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="row">
              <div class="form-goup">
                <div class="col-md-3 col-md-offset-4">                
                  <button type="button" ng-click="removeDialog('preview');" style="margin-right: -14px; width:72px;" class="button shining btn btn-md danger-inverted" >Cerrar</button>
                </div>
              </div>
            </div>
          </div>
  ...
  <div id="deletedSurvey" class="dialog">
      <div class="dialog__overlay"></div>
      <div class="dialog__content">
        <div class="morph-shape">
          <svg xmlns="https://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 560 280" preserveAspectRatio="none">
        ...
        <div class="dialog-inner">
          <h2 style="padding-bottom:35px">¿Estás seguro?</h2>
          <div>
            Si elimina esta encuesta no podrá recuperar los datos
          </div>
          <br>
          <div>
            <a onClick="closeModal();" class="button shining btn btn-md danger-inverted" data-dialog-close>Cancelar</a>
            <a href="#" onClick="hole();" id="btn-ok" class="button shining btn btn-md success-inverted">Aceptar</a>
          </div>
        </div>
      </div>
  </div>
...
<!-- <a href="https://www.youtube.com/playlist?list=PLruewKCYa5VsHIVGrihhG0Phf-tmqEk9m" target="_blank"> -->
  ...
  <script>
    ...
    function hole() {
      angular.element('#indexController').scope().deleteSurvey();
      angular.element('#indexController').scope().$apply();
    }
  </script>
{% endblock %}
{% block footer %}
