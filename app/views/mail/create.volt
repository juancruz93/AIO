...
{% block js %}
  ...
  {{ javascript_include('library/bootstrap-fileinput-master/js/fileinput.min.js') }}
  ...
  {#  <script src="//pineconellc.github.io/angular-foundation/mm-foundation-tpls-0.8.0.js"></script>#}
  ...
  {{ javascript_include('library/plupload-3.1.2/js/plupload.full.min.js')}}
{% endblock %}
...
{% block content %}
  ...
  <div ng-app="mail" >
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap">
        <div class="title">
          Crear nuevo envío de correo
        </div>
        <hr class="basic-line"/>
      </div>
    </div>

    <div id="form-container" ng-controller="main">

      <div class="text-center">
        <div id="status-buttons" class="" ui-sref-active="active">

          <a ng-class="{'wizard-select': describe}" 
             ui-sref="describe({id:idMailGet})" ui-sref-active="active"  >
            <span>1</span> <b>Información básica</b></a>

          <a ng-class="{'wizard-select': addressees}" ui-sref-active="active" 
             ng-href="{{"{{idMailGet != null ? '/mail/create#/addressees/' + idMailGet : ''}}" }}"
             ><span>2</span> <b>Destinatarios</b>
          </a>

          <a  ng-class="{'wizard-select': content}" 
              ng-href="{{"{{idMailGet != null ? '/mail/create#/content/' + idMailGet : ''}}" }}"
              ><span>3</span> <b>Contenido</b></a>
          <a ng-class="{'wizard-select': advanceoptions}" 
             ng-href="{{"{{idMailGet != null ? '/mail/create#/advanceoptions/' + idMailGet : ''}}" }}"
             ><span>4</span> <b>Opciones
              avanzadas</b></a>
          <a ng-class="{'wizard-select': shippingdate}" 
             ng-href="{{"{{idMailGet != null ? '/mail/create#/shippingdate/' + idMailGet : ''}}" }}"
             ><span>5</span> <b>Fecha de envío</b></a>
        </div>
      </div>
  ...
  {#<div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 wrap text-right">
          <a href="{{url('')}}" class="button shining btn btn-md success-inverted">Guardar</a>
          <a href="{{url('')}}" class="button shining btn btn-md danger-inverted">Cancelar</a>
      </div>
  </div>#}
