...
class HtmlZone extends HtmlAbstract {
  ...
  public function assignContent($zone) {
    ...
    if(!isset($_POST['typecontent'])){
        if (isset($this->account) && $this->account->AccountConfig->footerEditable == 0 && $zone->name === 'footer') {
          $footerObj = \Footer::findFirstByIdFooter($this->account->AccountConfig->idFooter);
          $footer = json_decode($footerObj->content);
          foreach ($footer->dz->footer->content as $frow) {
            $frow->widthval = 600;
            $row = new HtmlRow();
