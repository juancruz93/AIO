...
class ImageObject
{
	...
	public function createImageFromFile($path, $name)
	{
        ...
        if (!$this->img) {
          throw new \InvalidArgumentException("La imagen es nula");
        }
	}
	...
	public function resizeImage($w, $h, $color = null)
	{
            ...
            if ($color == null) {
                \imagecolortransparent($this->newImg, \imagecolorallocate($this->newImg, 0, 0, 0));
            }
            else {
	...
	public function saveImage($type, $path)
	{
		...
		if (!$img) {
                    throw new \InvalidArgumentException('No se pudo crear el thumbnail, por favor contacte al administrador');
		}	
	}
	...
	public function moveImageFromSiteToAnother ($tmp_dir, $dir) 
	{
		if (!move_uploaded_file($tmp_dir, $dir)){ 
                    throw new \InvalidArgumentException('No se pudo mover la imagen, por favor contacte al administrador');
		}
	}
}
