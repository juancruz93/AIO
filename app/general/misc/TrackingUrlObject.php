...
//		Phalcon\DI::getDefault()->get('logger')->log('Despúes: ' . print_r($this->links, true));
  ...
  public function getTrackingUrlForm($html, $form, $idContact) {
    $linkdecoder = $this->encoder;

    $action = 'api/contact/dobleoptin';
    $parameters = array(1, $form->idContactlist, $idContact, $form->idForm);
    ...
    $this->links['search'][] = '$$$_doble_optin_$$$';
...
//					Phalcon\DI::getDefault()->get('logger')->log('Dominio text: ' . $domain);
  ...
  public function getLinksFromContentMail($html, $text) {
    $imgTag = new \DOMDocument();
    @$imgTag->loadHTML($html);
    ...
    if ($hrefs->length !== 0) {
      foreach ($hrefs as $href) {
        $link = $href->getAttribute('href');
        $parts = parse_url($link);
        if (isset($parts['host']) || !empty($parts['host']) && isset($parts["scheme"])) {
