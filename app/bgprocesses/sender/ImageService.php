...
class ImageService {
  ...
  public function __construct(Account $account, Urldomain $domain, $urlManager) {
...
//			$idTemplateImage = filter_var($srcImg, FILTER_SANITIZE_NUMBER_INT);
  ...
  private function getCompletePrivateImageSrc($idAsset) {
    ...
    if (isset($asset->idAccount)) {
      ...
      $img = $this->domain->name . '/' . $this->urlManager->get_url_asset() . "/" . $this->account->idAccount . "/images/" . $asset->idAsset . "." . $ext;
//			$this->logger->log("Link final: {$img}");
      return $img;
    } else if (isset($asset->idAllied)) {
      $ext = pathinfo($asset->name, PATHINFO_EXTENSION);
      $img = $this->domain->name . '/' . $this->urlManager->get_url_asset_allied() . "/" . $asset->idAllied . "/images/" . $asset->idAsset . "." . $ext;
      return $img;
    } else {
//      $idAccount = $
      $ext = pathinfo($asset->name, PATHINFO_EXTENSION);
      $img = $this->domain->name . '/' . $this->urlManager->get_url_asset_root() . "/" . "/images/" . $asset->idAsset . "." . $ext;
//			$this->logger->log("Link final: {$img}");
      return $img;
    }
  }
