...
$lcc = new LevelContactsCounter();
$lcc->start($idAccount);

class LevelContactsCounter {
  ...
  public function start($idAccount) {
    ...
    $arrayContactEmail = array();
    $arrayContactPhone = array();
    foreach ($arrayGeneral as $key => $value) {
      ...
      if (($value["phone"] != null || $value["phone"] != "") && ($value["email"] == null || $value["email"] == "")) {
        array_push($arrayContactPhone, $value["phone"]);
        //Si solamente tiene email (no phone)
      } elseif (($value["phone"] == null || $value["phone"] == "") && ($value["email"] != null || $value["email"] != "")) {
        array_push($arrayContactEmail, $value["email"]);
      }
    }

    //Este procedimiento se modificó para que no tenga en cuenta los contactos con solo correo electronico
    //y asi evitar que se genere regreso al saldo original de los contactos o saldos negativos
